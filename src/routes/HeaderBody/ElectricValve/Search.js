      ...
      <Form onSubmit={this.handleSubmit} layout="inline" style={{overflow: 'hidden'}}>
        <Row >
          ...
          <FormItem label={formatMessage({id: 'intl.open_operating_bar'})} style={{float: 'right'}}>
            <Switch defaultChecked={localStorage.getItem('canOperateelectric_valves') === 'true' ? true : false}
                    onChange={(checked)=> {
                      localStorage.setItem('canOperateelectric_valves', checked);
