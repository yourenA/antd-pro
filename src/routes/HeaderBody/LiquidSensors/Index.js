...
class LiquidSensors extends PureComponent {
  ...
  scrollTable=()=>{
    ...
    if(scrollTop+offsetHeight>scrollHeight-300){
      if(this.state.canLoadByScroll){
        const {liquid_sensors: {meta}} = this.props;
  ...
  handleSearch = (values,cb,fetchAndPush=false) => {
    ...
    const that = this;
    dispatch({
      type:fetchAndPush?'liquid_sensors/fetchAndPush': 'liquid_sensors/fetch',
      payload: {
        ...values,
    ...
    return (
      <Layout className="layout">
        ...
        <Content >
          ...
          >
            <ConnectForm  editRecord={this.state.editRecord}
