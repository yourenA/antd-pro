...
class UserMeterAnalysis extends PureComponent {
  ...
  handleLeak = (is_save, value)=> {
    ...
    for (let i = 0; i < tbodyRow.length; i++) {
      ...
      if(forwardsInput.value===''||othersInput.value===''){
        console.log('存在空');
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        // console.log('record.layer*25',record.layer+25)
        return (
          <div><Input disabled={record.is_village === 1 ? false : true} onChange={()=> {
      ...
      {
        title: '远传用户总用水量',
        dataIndex: 'total_difference_value',
        key: 'total_difference_value',
        width: 140,
        render: (val, record, index) => {
          // console.log('record',record)
          // console.log('record.layer*25',record.layer+25)
      ...
      {
        title: '夜间用水量', dataIndex: 'night_difference_value', key: 'night_difference_value', render: (val, record, index) => {
        // console.log('record',record)
        // console.log('record.layer*25',record.layer+25)
        return (
          <span style={{paddingLeft: `${parseInt(record.layer) * 25}px`}}>{val}</span>
    ...
    const historyColumns = [
      ...
      {
        // fixed:'right',
        title: '开始日期', dataIndex: 'started_at', key: 'started_at', width: 100, render: (val, record, index) => {
        // console.log('record',record)
        // console.log('record.layer*25',record.layer+25)
        return (
          <span style={{paddingLeft: `${parseInt(record.layer) * 25}px`}}>{val}</span>
      ...
      {
        // fixed:'right',
        title: '结束日期', dataIndex: 'ended_at', key: 'ended_at',width: 100, render: (val, record, index) => {
        // console.log('record',record)
        // console.log('record.layer*25',record.layer+25)
        return (
          <span style={{paddingLeft: `${parseInt(record.layer) * 25}px`}}>{val}</span>
          ...
          <div className="content">
            <PageHeaderLayout title="实时数据分析" breadcrumb={[{name: '数据分析'}, {name: '小区水量分析'}]}>
              <Card bordered={false} style={{margin: '-16px -16px 0'}}>
                <Tabs defaultActiveKey="1" tabBarExtraContent={<Alert message="蓝色名称为监控表" type="info" showIcon/>}>
