        ...
        <Row>
          ...
          >
            {getFieldDecorator('display_type', {
              ...
              <RadioGroup>
                ...
                <RadioButton
                  value="only_missing_upload">{isToday ? formatMessage({id: 'intl.meter_no_upload_count_today'}) : formatMessage({id: 'intl.only_missing'})}</RadioButton>
          ...
          <FormItem>
            ...
            <Button type="primary" className="btn-cyan" style={{marginLeft: 8}} onClick={() => this.props.exportCSV()}
                    icon='export'>{formatMessage({id: 'intl.export_water_meter_readings'})}</Button>}
            {this.props.showConfigBtn && company_code !== 'hy' &&
            <Button type="primary" className="btn-cyan" style={{marginLeft: 8}}
                    onClick={() => this.props.setExport()}>{formatMessage({id: 'intl.set_export_format'})}</Button>}
            {company_code === 'll' && <Button type="primary" icon='upload' style={{marginLeft: 8}}
                                              onClick={() => this.props.uploadLl()}>上传醴陵读数</Button>}
          ...
          <FormItem label={formatMessage({id: 'intl.total_water_consumption'})}>
            <Input value={this.props.total_difference_value} style={{width: '173px'}} readOnly
                   addonAfter={formatMessage({id: 'intl.ton'})}/>
          </FormItem>
        </Row>
      </Form>
    )
