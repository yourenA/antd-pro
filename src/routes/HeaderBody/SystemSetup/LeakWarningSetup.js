...
import AddOrEditUnusualModels from './addOrEditLeakModels'
import AddOrEditUnusualSpecial from './addOrEditLeakSpecial'
...
class EditPassword extends Component {
    ...
    request(`/configs?groups[]=leak_abnormality`, {
      ...
      that.setState({
        ...
        leak_abnormality_meter_models: find(response.data.data, function (o) {
          return o.name === 'leak_abnormality_meter_models'
        }),
      ...
      (err, values) => {
        ...
        request(`/configs`, {
          ...
          if (response.status === 200) {
            ...
            message.success(`${formatMessage({id: 'intl.edit'})} ${this.state.leak_abnormality_value.display_name} ${formatMessage({id: 'intl.successful'})}`)
          }
        })
      }
    );
  ...
  handleSubmitModel = ()=> {
    ...
    this.setState({
      hadEditModel: true,
      modelModal: false,
      leak_abnormality_meter_models: this.state.leak_abnormality_meter_models
    })
  ...
  handleRemoveModel = (id)=> {
    const {intl:{formatMessage}} = this.props;
    const deleteIndex = findIndex(this.state.leak_abnormality_meter_models.value, function (o) {
      return o.id === id;
    });
    console.log('deleteIndex', deleteIndex)
    if (deleteIndex !== -1) {
      const leak_abnormality_meter_models = this.state.leak_abnormality_meter_models.value.splice(deleteIndex, 1)
      console.log(leak_abnormality_meter_models)
      console.log(this.state.leak_abnormality_meter_models.value)
  ...
  saveModel=()=>{
    ...
    request(`/configs`, {
      method: 'PATCH',
      data: {
        leak_abnormality_meter_models: this.state.leak_abnormality_meter_models.value
      }
    }).then((response)=> {
      console.log(response);
      if (response.status === 200) {
  ...
  handleRemoveSpecial = (number)=> {
    const that = this;
    const {intl:{formatMessage}} = that.props;
    const deleteIndex = findIndex(this.state.leak_abnormality_special_meters.value, function (o) {
      return o.number === number;
    });
    console.log('deleteIndex', deleteIndex)
    if (deleteIndex !== -1) {
      const leak_abnormality_special_meters = this.state.leak_abnormality_special_meters.value.splice(deleteIndex, 1)
      console.log(leak_abnormality_special_meters)
      console.log(this.state.leak_abnormality_special_meters.value)
      this.setState({
        hadEditSpecial: true,
        leak_abnormality_special_meters: this.state.leak_abnormality_special_meters
      })
  ...
  saveSpecial=()=>{
    ...
    request(`/configs`, {
      method: 'PATCH',
      data: {
        leak_abnormality_special_meters: this.state.leak_abnormality_special_meters.value
      }
    }).then((response)=> {
      console.log(response);
      if (response.status === 200) {
  ...
  render() {
    ...
    const that = this;
    const Special_dataSource = this.state.leak_abnormality_special_meters.value ? filter(this.state.leak_abnormality_special_meters.value, function (o) {
      return o.number.indexOf(that.state.searchInputValue) >= 0
    }) : []
    const Models_dataSource = this.state.leak_abnormality_meter_models.value ? this.state.leak_abnormality_meter_models.value : []
    return (
        ...
        <Content style={{background: '#fff'}}>
          <div className="content">
            <PageHeaderLayout title="系统管理" breadcrumb={[{name: formatMessage({id: 'intl.system'})},
              ...
              <Card bordered={false} style={{margin: '-16px -16px 0'}}>
                <Tabs defaultActiveKey="1" className="system-tabs alarm-tabs">
                  <TabPane tab={this.state.leak_abnormality_value.display_name} key="1">
           ...
           {/*       <TabPane tab={this.state.leak_abnormality_meter_models.display_name} key="2">
                ...
                <Modal
                  title={formatMessage({id: 'intl.add'}) + " " + this.state.leak_abnormality_meter_models.display_name}
                ...
                <Modal
                  ...
                  title={formatMessage({id: 'intl.edit'}) + " " + this.state.leak_abnormality_meter_models.display_name}
                ...
                <Modal
                  title={formatMessage({id: 'intl.add'}) + " " + this.state.leak_abnormality_special_meters.display_name}
                ...
                <Modal
                  ...
                  title={formatMessage({id: 'intl.edit'}) + " " + this.state.leak_abnormality_special_meters.display_name}
