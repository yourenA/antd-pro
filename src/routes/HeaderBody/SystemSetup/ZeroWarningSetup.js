...
import AddOrEditUnusualModels from './addOrEditZeroModels'
import AddOrEditUnusualSpecial from './addOrEditZeroSpecial'
...
class EditPassword extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      zero_abnormality_meter_models: {},
      zero_abnormality_special_meters: {},
      searchInputValue: '',
      searchSpecialValue: '',
      tableY: 0,
      hadEditModel: false,
  ...
  componentDidMount() {
    ...
    request(`/configs?groups[]=zero_abnormality`, {
      ...
      that.setState({
        ...
        zero_abnormality_meter_models: find(response.data.data, function (o) {
          return o.name === 'zero_abnormality_meter_models'
        }),
        zero_abnormality_special_meters: find(response.data.data, function (o) {
          return o.name === 'zero_abnormality_special_meters'
        }),
      }, function () {
      ...
      (err, values) => {
        ...
        request(`/configs`, {
          ...
          if (response.status === 200) {
            ...
            message.success(`${formatMessage({id: 'intl.edit'})} ${this.state.zero_abnormality_days.display_name} ${formatMessage({id: 'intl.successful'})}`)
          }
        })
      }
    );
  ...
  handleSubmitModel = ()=> {
    ...
    for (let i = 0; i < formValues.meter_model_ids.length; i++) {
      const isExit = find(this.state.zero_abnormality_meter_models.value, function (o) {
    ...
    this.state.zero_abnormality_meter_models.value = [...newModels, ...this.state.zero_abnormality_meter_models.value]

    this.setState({
      hadEditModel: true,
      modelModal: false,
      zero_abnormality_meter_models: this.state.zero_abnormality_meter_models
    })
  ...
  handleSubmitEditModel = ()=> {
    ...
    if (formValues.days === undefined) {
      message.error(`异常判断连续天数${formatMessage({id: 'intl.can_not_be_empty'})}`)
      return false
    }
    const that = this;
    const editIndex = findIndex(this.state.zero_abnormality_meter_models.value, function (o) {
      return o.id === that.state.editModelRecord.id;
    });
    console.log('editIndex', editIndex)
    this.state.zero_abnormality_meter_models.value[editIndex].days = formValues.days;
  ...
  handleRemoveModel = (id)=> {
    const {intl:{formatMessage}} = this.props;
    const deleteIndex = findIndex(this.state.zero_abnormality_meter_models.value, function (o) {
      return o.id === id;
    });
    console.log('deleteIndex', deleteIndex)
    if (deleteIndex !== -1) {
      const zero_abnormality_meter_models = this.state.zero_abnormality_meter_models.value.splice(deleteIndex, 1)
      console.log(zero_abnormality_meter_models)
      console.log(this.state.zero_abnormality_meter_models.value)
  ...
  saveModel = ()=> {
    forEach(this.state.zero_abnormality_meter_models.value, function (item, index) {
      delete item.name
    });
    const that = this
    request(`/configs`, {
      method: 'PATCH',
      data: {
        zero_abnormality_meter_models: this.state.zero_abnormality_meter_models.value
      }
    }).then((response)=> {
      console.log(response);
      if (response.status === 200) {
  ...
  handleSubmitSpecial = ()=> {
    ...
    for (let i = 0; i < formValues.meter_numbers.length; i++) {
      const isExit = find(this.state.zero_abnormality_special_meters.value, function (o) {
    ...
    this.state.zero_abnormality_special_meters.value = [...newMeters, ...this.state.zero_abnormality_special_meters.value]

    this.setState({
      hadEditSpecial: true,
      specialModal: false,
      zero_abnormality_special_meters: this.state.zero_abnormality_special_meters
    })
  ...
  handleRemoveSpecial = (number)=> {
    const that = this;
    const {intl:{formatMessage}} = that.props;
    const deleteIndex = findIndex(this.state.zero_abnormality_special_meters.value, function (o) {
      return o.number === number;
    });
    console.log('deleteIndex', deleteIndex)
    if (deleteIndex !== -1) {
      const zero_abnormality_special_meters = this.state.zero_abnormality_special_meters.value.splice(deleteIndex, 1)
      console.log(zero_abnormality_special_meters)
      console.log(this.state.zero_abnormality_special_meters.value)
      this.setState({
        hadEditSpecial: true,
        zero_abnormality_special_meters: this.state.zero_abnormality_special_meters
      })
  ...
  handleSubmitEditSpecial = ()=> {
    ...
    const editIndex = findIndex(this.state.zero_abnormality_special_meters.value, function (o) {
      return o.number === that.state.editSpecialRecord.number;
    });
    console.log('editIndex', editIndex)
    this.state.zero_abnormality_special_meters.value[editIndex].days = formValues.days;
    this.setState({
      hadEditSpecial: true,
  ...
  saveSpecial = ()=> {
    const that = this
    forEach(this.state.zero_abnormality_special_meters.value, function (item, index) {
      delete item.name;
      delete item.id
    });
    request(`/configs`, {
      method: 'PATCH',
      data: {
        zero_abnormality_special_meters: this.state.zero_abnormality_special_meters.value
      }
    }).then((response)=> {
      console.log(response);
      if (response.status === 200) {
  ...
  render() {
    ...
    const that = this;
    const Special_dataSource = this.state.zero_abnormality_special_meters.value ? filter(this.state.zero_abnormality_special_meters.value, function (o) {
      return o.number.indexOf(that.state.searchInputValue) >= 0
    }) : []
    const Models_dataSource = this.state.zero_abnormality_meter_models.value ? this.state.zero_abnormality_meter_models.value : []
    return (
            ...
            <PageHeaderLayout title="系统管理" breadcrumb={[{name: formatMessage({id: 'intl.system'})},
              ...
              <Card bordered={false} style={{margin: '-16px -16px 0'}}>
                <Tabs defaultActiveKey="1" className="system-tabs alarm-tabs">
                  <TabPane tab={this.state.zero_abnormality_days.display_name} key="1">
                  ...
                  <TabPane tab={this.state.zero_abnormality_meter_models.display_name} key="2">
                    <div>
                           ...
                           style={{background: this.state.hadEditModel ? '#fce4d6' : '#e2efda'}}>
                        {
                          Models_dataSource.map((item, index)=> {
                            return <div key={index} className="alarm-item">
                              ...
                              <p className="alarm-item-value">异常判断连续天数 : <span >{item.days}</span></p>
                  ...
                  <TabPane tab={this.state.zero_abnormality_special_meters.display_name} key="3"
                ...
                <Modal
                  title={formatMessage({id: 'intl.add'}) + " " + this.state.zero_abnormality_meter_models.display_name}
                ...
                <Modal
                  ...
                  title={formatMessage({id: 'intl.edit'}) + " " + this.state.zero_abnormality_meter_models.display_name}
                ...
                <Modal
                  title={formatMessage({id: 'intl.add'}) + " " + this.state.zero_abnormality_special_meters.display_name}
                ...
                <Modal
                  ...
                  title={formatMessage({id: 'intl.edit'}) + " " + this.state.zero_abnormality_special_meters.display_name}
